...
def _error_page(title: str, exc: RevisionStudioError, back: str) -> HTMLResponse:
    response = page(
        ...
        <div class="revision-nav">
          <a class="button secondary" href="{html.escape(back, quote=True)}">Back</a>
        </div>
...
def _document_summaries(root: Path) -> list[dict[str, object]]:
...
def revision_index() -> HTMLResponse:
    cards: list[str] = []
    for item in _document_summaries(revision_root()):
        ...
        cards.append(
            ...
            <div class="card">
              ...
              <p>{_safe(item['open_count'])} open proposal(s); {_safe(item['proposal_count'])} retained total.</p>
    ...
    content = "".join(cards) or (
        '<div class="card"><p class="muted">No revision documents yet.</p></div>'
    )
    return page(
        "Revision Studio",
        EXTRA_STYLE
...
def revision_document(document_id: str) -> HTMLResponse:
    ...
    proposals_html = "".join(proposal_cards) or (
...
def revision_compare(document_id: str, proposal_id: str) -> HTMLResponse:
    ...
    metadata = "".join(
        (
            _metadata_list("Evidence", list(proposal.get("evidence_refs", []))),
            _metadata_list("Validation", list(proposal.get("validation_refs", []))),
            _metadata_list(
                "Consequences", list(proposal.get("consequence_notes", []))
            ),
            _metadata_list("Risks", list(proposal.get("risk_notes", []))),
        )
    )
    diff = (
        _safe(comparison["unified_diff"])
        if comparison is not None
        else "Comparison unavailable because the proposal is stale."
    )
    ...
    if proposal.get("state") == "proposed" and comparison is not None:
        ...
        accept_all = post_form(
            action_url, _decision_fields("accept_all", "Accept all")
        )
        keep = post_form(
            action_url,
        ...
        reject = post_form(
            action_url, _decision_fields("reject", "Reject", "danger")
        )
        checkboxes = "".join(
        ...
        actions = (
            '<div class="card revision-actions"><h2>Decision</h2>'
            f"{accept_all}{partial}{keep}{reject}</div>"
        )
