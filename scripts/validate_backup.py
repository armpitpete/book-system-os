...
ARCHIVE_REASON_RE = re.compile(
    r"^Archived by cleanup helper; older than ([1-9][0-9]*) days$"
)
REQUIRED_JOB_DIRECTORIES = ("input", "work", "output", "logs")
EVENT_HISTORY_REQUIRED_AT = datetime(
    2026, 6, 21, 10, 33, 9, tzinfo=timezone.utc
)
LEGACY_STATE_SETTER_INTRODUCED_AT = datetime(
    2026, 6, 21, 8, 8, 10, tzinfo=timezone.utc
)
EVIDENTIAL_CLEANUP_AVAILABLE_AT = datetime(
    2026, 6, 21, 13, 52, 38, tzinfo=timezone.utc
)
FINAL_MANIFEST_STATUS_REQUIRED_AT = datetime(
    2026, 7, 19, 18, 56, 19, tzinfo=timezone.utc
)
    ...
    def as_dict(self) -> dict[str, object]:
        return {
            ...
            "legacy_manifests_without_final_status": (
                self.legacy_manifests_without_final_status
            ),
...
def _safe_member_path(name: str) -> PurePosixPath:
    ...
    if path != BACKUP_PREFIX and BACKUP_PREFIX not in path.parents:
        raise BackupValidationError(
            f"Archive member is outside {BACKUP_PREFIX}: {name!r}"
        )
    return path
...
def _parse_utc_timestamp(value: object, name: str) -> datetime:
    ...
    try:
        ...
        raise BackupValidationError(
            f"Timestamp is missing or invalid: {name}"
        ) from exc
...
def _validate_events(raw: bytes, name: str) -> list[dict[str, object]]:
    try:
        ...
        raise BackupValidationError(
            f"Event history is not UTF-8: {name}"
        ) from exc
    ...
    for number, line in enumerate(lines, start=1):
        ...
        try:
            ...
            raise BackupValidationError(
                f"Invalid event JSON in {name}:{number}: {exc}"
            ) from exc
        if not isinstance(event, dict):
            raise BackupValidationError(
                f"Event must be an object in {name}:{number}"
            )
        events.append(event)
...
def _validate_config_shape(raw: bytes) -> None:
    ...
    for key in keys:
        if "=" in key or not ENV_KEY_RE.fullmatch(key):
            raise BackupValidationError(
                f"config/env.keys contains a value or invalid key: {key!r}"
            )
...
def _validate_manifest_outputs(
    ...
    if not isinstance(outputs, dict) or not outputs:
        raise BackupValidationError(
            f"Completed job manifest has no outputs: {job_id}"
        )
    ...
    for output_name in outputs.values():
        if (
            ...
            raise BackupValidationError(
                f"Unsafe manifest output name in job {job_id}"
            )
        output_path = job_root / "output" / output_name
        output_member = members.get(output_path)
        if (
            output_member is None
            or not output_member.isfile()
            or output_member.size <= 0
        ):
            raise BackupValidationError(
                f"Manifest output is missing or empty: {output_path}"
            )
...
def _is_cleanup_archived_test_transition(
    *,
    ...
    archived_at = _parse_utc_timestamp(
        status.get("archived_at"), f"{job_id}/status.json archived_at"
    )
    updated_at = _parse_utc_timestamp(
        status.get("updated_at"), f"{job_id}/status.json updated_at"
    )
    ...
    for event_number, event in enumerate(events, start=1):
        if (
            ...
            or event.get("message") != reason
            or event.get("older_than_days") != older_than_days
        ):
            continue
        event_at = _parse_utc_timestamp(
            event.get("created_at"),
            f"{job_id}/events.jsonl archived event {event_number} created_at",
        )
...
def _is_legacy_manual_archived_test_transition(
    *,
    ...
    manifest_completed_at: datetime,
    manifest_status: dict[str, object],
    status: dict[str, object],
    events: list[dict[str, object]],
    job_id: str,
) -> bool:
    if status.get("state") != "archived" or manifest_status.get("state") != "test":
        return False
    if status.get("status") != "done":
        return False
    if status.get("step") != "complete" or status.get("message") != "Build complete":
        return False
    if (
        manifest_status.get("status") != "running"
        or manifest_status.get("step") != LEGACY_MANIFEST_STEP
        or manifest_status.get("message") != LEGACY_MANIFEST_MESSAGE
    ):
        return False
    if status.get("archived_at") is not None or status.get("archive_reason") is not None:
        return False
    if any(event.get("event") == "archived" for event in events):
        return False
    ...
    if not (
        LEGACY_STATE_SETTER_INTRODUCED_AT
        <= metadata_created_at
        <= manifest_completed_at
        <= updated_at
        < EVIDENTIAL_CLEANUP_AVAILABLE_AT
    ):
        return False
    if updated_at - manifest_completed_at > timedelta(minutes=5):
        return False

    manifest_updated_at = _parse_utc_timestamp(
        manifest_status.get("updated_at"),
        f"{job_id}/manifest.json status.updated_at",
    )
    if manifest_updated_at > manifest_completed_at:
        return False
    ...
    if (
        ...
        or retry_count < 1
        or manifest_status.get("retry_count") != retry_count
    ):
        return False

    retry_at = _parse_utc_timestamp(
        status.get("last_retry_at"), f"{job_id}/status.json last_retry_at"
    )
    manifest_retry_at = _parse_utc_timestamp(
        manifest_status.get("last_retry_at"),
        f"{job_id}/manifest.json status.last_retry_at",
    )
    if retry_at != manifest_retry_at:
        return False
    if not (metadata_created_at <= retry_at <= manifest_updated_at):
        return False

    for event_number, event in enumerate(events, start=1):
        if event.get("event") != "retry" or event.get("retry_count") != retry_count:
            continue
        event_at = _parse_utc_timestamp(
            event.get("created_at"),
            f"{job_id}/events.jsonl retry event {event_number} created_at",
        )
        if event_at == retry_at:
            return True
...
def _validate_manifest_state(
    *,
    ...
    if (
        ...
        or not isinstance(embedded_state, str)
        or external_state == embedded_state
    ):
        return None

    if _is_cleanup_archived_test_transition(
        status=status,
    ...
    if _is_legacy_manual_archived_test_transition(
    ...
    raise BackupValidationError(
        f"Manifest state does not match final status: {job_id}"
    )
...
def _validate_completed_manifest(
    ...
    manifest_completed_at = _parse_utc_timestamp(
        manifest.get("completed_at"), f"{job_id}/manifest.json completed_at"
    )
    manifest_status = manifest.get("status")
    if not isinstance(manifest_status, dict):
        raise BackupValidationError(
            f"Completed job manifest has invalid status: {job_id}"
        )

    legacy_manifest = manifest_status.get("status") != "done"
    if legacy_manifest:
        legacy_shape = (
            manifest_completed_at < FINAL_MANIFEST_STATUS_REQUIRED_AT
        ...
        if not legacy_shape:
            raise BackupValidationError(
                f"Completed job manifest is not final: {job_id}"
            )

    transition = _validate_manifest_state(
        metadata_created_at=metadata_created_at,
        manifest_completed_at=manifest_completed_at,
        manifest_status=manifest_status,
        status=status,
    ...
    return legacy_manifest, transition == "legacy-manual"
...
def _validate_job(
    ...
    if metadata.get("job_id") != job_id:
        raise BackupValidationError(
            f"Job identity mismatch: directory {job_id!r}, "
            f"metadata {metadata.get('job_id')!r}"
        )
    ...
    metadata_created_at = _parse_utc_timestamp(
    ...
    if events_member is None:
        if metadata_created_at >= EVENT_HISTORY_REQUIRED_AT:
            raise BackupValidationError(
        ...
        raise BackupValidationError(
            f"Event history path is not a file: {events_path}"
        )
    else:
    ...
    if status.get("status") == "done":
        ...
        _parse_json_object(
            _read_member_bytes(archive, manifest_member),
            f"{job_id}/manifest.json",
        )
...
def validate_archive(archive_path: Path) -> ValidationSummary:
    ...
    try:
        ...
        raise BackupValidationError(
            f"Backup archive cannot be opened: {exc}"
        ) from exc
    ...
    with archive:
        ...
        if not isinstance(metadata.get("created_at"), str) or not metadata.get(
            "created_at"
        ):
            raise BackupValidationError(
                "Backup metadata has no creation timestamp"
            )
        ...
        for path in members:
            if path in forbidden or any(
                parent in forbidden for parent in path.parents
            ):
                raise BackupValidationError(
                    f"Forbidden backup content is present: {path}"
                )
            if path.name == ".lock":
                raise BackupValidationError(
                    f"Ephemeral lock file is present: {path}"
                )
...
def _ensure_empty_destination(destination: Path) -> None:
    if destination.exists():
        if not destination.is_dir():
            raise BackupValidationError(
                f"Restore destination is not a directory: {destination}"
            )
        if any(destination.iterdir()):
            raise BackupValidationError(
                f"Restore destination is not empty: {destination}"
            )
    else:
    ...
    try:
        with tarfile.open(archive_path, mode="r:*") as archive:
            ...
            for archive_path_key, member in ordered:
                ...
                if (
                    ...
                    and destination not in target_resolved.parents
                ):
                    raise BackupValidationError(
                ...
                extracted_hashes[target] = _restore_member(
        ...
        for path, expected_hash in extracted_hashes.items():
            if _sha256_file(path) != expected_hash:
                raise BackupValidationError(
                    f"Restored file hash mismatch: {path}"
                )
        if (destination / "config" / "env").exists() or (
            destination / ".env"
        ).exists():
            raise BackupValidationError(
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Validate a Book System OS job-store backup and optionally "
            "restore it safely."
        )
    )
    parser.add_argument(
        "archive", type=Path, help="Path to the .tar.gz backup archive"
    )
    parser.add_argument(
        "--restore-root",
        type=Path,
        help=(
            "Optional absent or empty destination for a clean-system "
            "restore rehearsal"
        ),
    )
