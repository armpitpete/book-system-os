...
from app.pipeline.image_holders import ImageHolderError, holder_from_attributes, validate_image_holder
...
def _image_parts(node: dict[str, Any]) -> tuple[str, str, dict[str, str]] | None:
    ...
    alt_text = _inline_text(caption)
    return target[0], alt_text, attributes
...
def validate_local_image_files(markdown: str, *, source_dir: Path) -> None:
    ...
    Plain Markdown images retain existing compatibility and are checked for
    existence. Images with a ``holder`` attribute also receive deterministic
    geometry, format, accessibility and print-resolution validation.
    """
